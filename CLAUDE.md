...
**One-off scripts get the right database automatically — but verify anyway.** `app.py` calls `load_dotenv()` itself, anchored to its own file location (`os.path.dirname(os.path.abspath(__file__))`), so any script anywhere that does `from app import app, db` gets `DATABASE_URL` loaded correctly regardless of the script's own location or working directory. This exists because the naive pattern — a one-off script calling bare `load_dotenv()` itself before importing `app` — is a footgun: python-dotenv's default `load_dotenv()` searches from the *calling script's own file location*, not the working directory, so it fails silently for any script that isn't co-located with `.env` (e.g. one written to a temp/scratch directory) — and `app.py`'s `SQLALCHEMY_DATABASE_URI` line falls back to local SQLite with no exception, so the script "succeeds" against the wrong database. If `DATABASE_URL` is still missing at startup, `app.py` now prints a `WARNING` to stderr — if you don't see that line, you're on Supabase. **Before trusting any one-off script's output**, print `db.engine.url` inside `app.app_context()` and diff a row count before/after — don't rely on "0 errors" as proof it hit production. See `Design/design.md` → "Known Issues Fixed" for the incident this fixed.
