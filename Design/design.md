...
DB category name: `Annual eResources` (renamed from `eResources`; code that looks it up by name must use the new name). `frequency='annual'` (fixed from an earlier `'monthly'` setting — this is what makes the entry form hide the Month field and show only a Fiscal Year selector). Metrics: E-Book Circulation, E-Audio Circulation, E-Video Circulation, E-Serials Circulation — entered once per fiscal year via the normal `/entries/new/<category_id>` form. Because `has_branch=False`, entries have `branch_id=None` (system-wide, same convention as Online Stats) — **not** `branch_id` of the `YCL (System Wide)` Branch row; the `ac_totals()`/`_fy_totals()` helpers in `app.py` (Annual Comparables report) and the Director's Dashboard's `fy_filter_annual()` all filter on `branch_id=None, year=<FY ending year>, month=None`. First real data (FY2025-2026) was loaded via `patch_fy2526_annual_eresources.py`, sourced from the SC State Annual Report electronic-usage export. Feeds the eBook/eAudio/eVideo/eSerial Circ rows (H14-H17) on the Director's Dashboard and the "digital" total in the Annual Comparables report. Still shows "No data" in the dashboard Data Status widget for any fiscal year that hasn't been loaded — a visible reminder, not a bug.
...
| `Data files/annual/YCL_Stats_2025.xlsx` | 2026-04-27 | 84 Branch Stats entries updated — Jul 2024 – Jun 2025 (12 months, all branches). **⚠ Suspected to have loaded Total Branch Circulation as checkouts-only (missing renewals) — see `Design/incident_2026-08-04_report_verification.md`. FY2025 circulation total (596,933) undercounts the verified state-survey figure (1,014,285) by almost exactly the checkouts-only share of transactions. Not yet fixed — file no longer exists locally to reload correctly.** |
