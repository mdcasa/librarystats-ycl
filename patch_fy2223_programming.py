"""
Patch FY2022-23 programming data from '22-23 Annual Stats.xls'.

Source: 'Program Comparison p.5' sheet.

Convention (matches FY23-24 and FY24-25 patch scripts):
...
Creates Branch Stats entries where none exist; skips any branch/month that
already has non-zero programming data.
"""
...
XLS = 'Data files/Ls/22-23 Annual Stats.xls'
CATEGORY_ID = 1  # Branch Stats

# Column index (0-based) → calendar month/year for FY July 2022 – June 2023
COL_TO_MONTH = {3:7, 4:8, 5:9, 6:10, 7:11, 8:12, 9:1, 10:2, 11:3, 12:4, 13:5, 14:6}
COL_TO_YEAR  = {3:2022, 4:2022, 5:2022, 6:2022, 7:2022, 8:2022,
                9:2023, 10:2023, 11:2023, 12:2023, 13:2023, 14:2023}
...
# Rows verified against 'Program Comparison p.5' in 22-23 Annual Stats.xls
SECTIONS = [
...
def main(dry_run=True):
    print(f"{'DRY RUN — ' if dry_run else ''}Loading FY2022-23 programming data\n")
                ...
                if not dry_run:
                    if not entry:
                        entry = Entry(
                            ...
                            year=year,
                            month=month,
                            submitted_by='Patch: FY2223 programming',
                        )
