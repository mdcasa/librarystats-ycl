...
def seed_eresource_databases(db):
    ...
    crosswalk in eResources/YCL-Vendor-Data-Onboarding-Plan.md §3a — None means
    not counted for the state report, tracked for internal reference only.

    Worked out vendor-by-vendor against the director's tracking workbook
    ('Monthly Stats 2025-2026.xlsx'). Source sheet/column for each entry is
    recorded here as a comment for import_eresources.py.
    """
    entries = [
        # (name, vendor, bucket)
        # ABC Mouse — sheet 'ABC Mouse'
        # NOTE: the sheet's own footer says "For State Report, report Learning
        # Activities" but that's wrong — Learning Activities totals 41,555
        # (nearly 9x the entire E-Video bucket target of 31,483), while Visits
        # totals 4,793, which exactly reconciles against the Annual Stats for
        # SC State export. Bucket assignment follows the reconciled math, not
        # the sheet's own (incorrect) instructions.
        ('ABC Mouse - Learning Activities', 'ABC Mouse', None),          # col: Learning Activities
        ('ABC Mouse - Visits',              'ABC Mouse', 'evideo'),     # col: Visits
